...
export default async function MoleculesPage ({ params }) {
	const { molecule } = await params
	const data = Molecules.find(a => a.id === molecule);

	if (!molecule || !data) return <div>Perro no encontrado</div>;

	return (
		<div className='section'>
			<article className='flex flex-col md:flex-row gap-3 items-center'>
				<section className='grow'>
					<h1 className='text-3xl font-medium'>Molecules - {data.name}</h1>
					<p className='max-w-[80ch] text-pretty mt-1 text-zinc-900/80 dark:text-white/80 text-[15px]'>
						Molecules are the smallest components that can be used to create more complex components. {data.description}
					</p>
				</section>
			...
			<article className='grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-4 mt-6'>
				{
					[...Array(20)].map((_, index) => (
						<section key={index} className='rounded-lg dark:bg-zinc-900 card-border overflow-hidden group'>
							<Link href={`/molecules/${molecule}/${index}`} className='flex flex-col overflow-hidden aspect-video'>
